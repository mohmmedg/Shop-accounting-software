...
export const CustomerAndSupplierView: React.FC = () => {
  ...
  const handleCustSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSavingCustomer || isUpdatingCustomer) return; // منع الإرسال المزدوج
    try {
      if (editingCustomer) {
  ...
  const handleSuppSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSavingSupplier || isUpdatingSupplier) return; // منع الإرسال المزدوج
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-4" id="modal-customer-add-edit" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-right">
            ...
            <form onSubmit={handleCustSave} className="space-y-4 font-bold text-xs text-slate-300">
              ...
              <button
                type="submit"
                disabled={isSavingCustomer || isUpdatingCustomer}
                className="w-full bg-indigo-600 hover:bg-indigo-550 text-white font-black py-3 rounded-xl shadow-lg transition active:scale-95 cursor-pointer mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {(isSavingCustomer || isUpdatingCustomer) ? 'جاري الحفظ...' : 'حفظ بيانات العميل'}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-right">
            ...
            <form onSubmit={handleSuppSave} className="space-y-4 font-bold text-xs text-slate-300">
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-550 text-white font-black py-3 rounded-xl shadow-lg transition active:scale-95 cursor-pointer mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {(isSavingSupplier || isUpdatingSupplier) ? 'جاري الحفظ...' : 'تسجيل وحفظ المورد'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-4" id="debt-amount-edit-modal" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-right space-y-4">
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 text-[10px] font-bold text-amber-400 leading-relaxed">
              ⚠️ هذا تصحيح إداري مباشر على مبلغ الدين (مثل إسقاط جزء منه أو تصحيح خطأ). لا يُسجَّل أي مبلغ محصَّل فعلياً في صندوق الكاش، لكن سيتم تخفيض "الربح المسجّل" لهذه الفاتورة بنفس مقدار المبلغ المُسقط تلقائياً — لأن هذا المبلغ لن يُحصَّل. أما "حجم المشتريات" فيبقى كما هو لأنه يعكس البضاعة التي اشتراها العميل فعلاً، وليس ما دفعه.
            </div>
